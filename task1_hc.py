...
def find_most_Queens_hill_climbing(m, n, max_steps=1000, alpha=10):
    ...
    def cost(board, attack_cnt):
        ...
        for i in range(m):
            for j in range(n):
                if board[i][j] == 'Q':
                    # attack_cnt[i][j] 表示這顆 queens 被其他 queens 攻擊的次數
                    conflict += attack_cnt[i][j]
        conflict //= 2  # 每對衝突會被算兩次
        # 用 attack_cnt 計算安全格數：
        safe_count = 0
        safe_count = sum(1 for i in range(m) for j in range(n) if board[i][j] == '.' and attack_cnt[i][j] == 0)
        # print(f"cost: num_queenss={num_queenss}, conflict={conflict}, safe_count={safe_count}")
        return -num_queens + alpha * conflict + safe_count  # safe_count 權重可再調整
